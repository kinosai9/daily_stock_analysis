...
These hit the REAL TWSE T86 + TPEx OpenAPI endpoints, so they run only when invoked
explicitly with `pytest -m network`. The blocking backend gate runs
`pytest -m "not network"` (scripts/ci_gate.sh), so these never gate a PR.
